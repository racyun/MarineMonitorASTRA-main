...
class MaskedAutoencoderGroupChannelViT(nn.Module):
    ...
    def forward(self, imgs, imgs2, mask_ratio=0.95):
        latent, mask, ids_restore = self.forward_encoder(imgs, 0)
        latent2, mask2, ids_restore2 = self.forward_encoder(imgs2, mask_ratio)
        pred = self.forward_decoder(torch.mean(latent, latent2), ids_restore)  # [N, C, L, p*p]
        loss = self.forward_loss(imgs2, pred, mask)
