...
class MaskedAutoencoderViT(nn.Module):
    ...
    def random_masking(self, x, mask_ratio, mask=None):
        ...
        if self.same_mask:
            ...
            ids_shuffle = [ids_shuffle + i * L2 for i in range(2)]
    ...
    def forward_encoder(self, x, timestamps, mask_ratio, mask=None):
        ...
        ts_embed = torch.cat([get_1d_sincos_pos_embed_from_grid_torch(128, timestamps.reshape(-1, 2)[:, 0].float()),
                   get_1d_sincos_pos_embed_from_grid_torch(128, timestamps.reshape(-1, 2)[:, 1].float())], dim=1).float()
        
        # print(ts_embed, ts_embed.shape)
        
        ts_embed = ts_embed.reshape(-1, 2, ts_embed.shape[-1]).unsqueeze(2)
        # print(ts_embed.shape)
        ts_embed = ts_embed.expand(-1, -1, x.shape[1] // 2, -1).reshape(x.shape[0], -1, ts_embed.shape[-1])
        # print(ts_embed.shape)
    ...
    def forward_decoder(self, x, timestamps, ids_restore):
        ...
        ts_embed = torch.cat([get_1d_sincos_pos_embed_from_grid_torch(64, timestamps.reshape(-1, 2)[:, 0].float()),
                   get_1d_sincos_pos_embed_from_grid_torch(64, timestamps.reshape(-1, 2)[:, 1].float())], dim=1).float()
